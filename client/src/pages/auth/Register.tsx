...
export default function Register() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      setServerError(error.message || 'Đăng ký bằng Google thất bại');
    }
  };
      ...
      <div className="w-full max-w-md bg-white/10 backdrop-blur-lg border border-white/20 shadow-2xl rounded-3xl p-8 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div>
              <label className="block text-white/90 text-sm font-medium mb-1.5" htmlFor="displayName">Tên hiển thị</label>
              <input
                id="displayName"
                type="text"
              ...
              />
              {errors.displayName && <p className="text-red-200 text-sm mt-1.5">{errors.displayName.message}</p>}
            </div>
            ...
            <button
              ...
              className="w-full py-3.5 bg-white text-red-600 hover:bg-white/90 rounded-xl font-semibold shadow-lg transition-all transform active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed mt-2"
            >
